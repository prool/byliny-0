...
void sedit_types_menu(struct descriptor_data *d, int mode);
...
void sedit_setup_new(struct descriptor_data *d)
{
  ...
  S_BUY(shop)     = str_dup("%s Вот тебе %d кун.");
  S_SELL(shop)    = str_dup("%s Пожалуй, я заплачу за это %d кун.");
  /*
  ...
  for (i = zone * 100; i <= top; i++) 
      {if ((rshop = real_shop(i)) != -1) 
          {shop = shop_index + rshop;
           fprintf(shop_file, "#%d%s~\n", i, (S_CHANGELISTS(shop) && S_CHANGETYPE(shop,0) != -1) ? " E" : "");
           /*
            * Save the products.
            */
...
void sedit_changelist_menu(struct descriptor_data *d)
{
  struct shop_data *shop;
  int i;
...
void sedit_disp_menu(struct descriptor_data *d)
{
  ...
  sprintf(buf,
	  ...
	  grn, nrm,
	  grn, nrm,
	  grn, nrm, cyn, S_CHANGEPROFIT(shop),
	  grn, nrm
	  );
  send_to_char(buf, d->character);
