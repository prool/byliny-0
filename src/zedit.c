...
void zedit_disp_menu(struct descriptor_data *d)
{
  ...
  sprintf(buf,
	  ...
	  "%sT%s) Макс.комната: %s%d\r\n"
	  "%sR%s) Тип очистки : %s%s%s\r\n"
	  "[Command list]\r\n",
  ...
  { /*
    ...
    {case 'M':
          sprintf(buf2, "%sзагрузить %s [%s%d%s], Max(в игре) : %d, Max(в комнате) : %d",
	              MYCMD.if_flag ? " тогда " : "",
      ...
      break;
     case 'F':
          sprintf(buf2, "%sследовать за %s [%s%d%s] %s [%s%d%s]",
	              MYCMD.if_flag ? " тогда " : "",
      ...
      break;
     case 'Q':
          sprintf(buf2, "%sубрать всех мобов %s [%s%d%s]",
	              MYCMD.if_flag ? " тогда " : "",
	              mob_proto[MYCMD.arg1].player.short_descr,
	              cyn, mob_index[MYCMD.arg1].vnum, yel
	             );
      break;
    case 'G':
      sprintf(buf2, "%sдать это %s [%s%d%s], Max : %d",
	      MYCMD.if_flag ? " тогда " : "",
      ...
      break;
    case 'O':
      sprintf(buf2, "%sзагрузить %s [%s%d%s], Max : %d",
	      MYCMD.if_flag ? " then " : "",
	      obj_proto[MYCMD.arg1].short_description,
	      cyn, obj_index[MYCMD.arg1].vnum, yel,
	      MYCMD.arg2
      ...
      break;
    case 'E':
      sprintf(buf2, "%sэкипировать %s [%s%d%s], %s, Max : %d",
	      MYCMD.if_flag ? " тогда " : "",
	      obj_proto[MYCMD.arg1].short_description,
	      cyn, obj_index[MYCMD.arg1].vnum, yel,
      ...
      break;
    case 'P':
      sprintf(buf2, "%sпоместить %s [%s%d%s] в %s [%s%d%s], Max : %d",
	      MYCMD.if_flag ? " тогда " : "",
	      obj_proto[MYCMD.arg1].short_description,
	      cyn, obj_index[MYCMD.arg1].vnum, yel,
      ...
      break;
    case 'R':
      sprintf(buf2, "%sудалить %s [%s%d%s] из комнаты.",
	      MYCMD.if_flag ? " then " : "",
      ...
      break;
    case 'D':
      sprintf(buf2, "%sустановить выход %s как %s.",
	      MYCMD.if_flag ? " then " : "",
	      dirs[MYCMD.arg2],
	      MYCMD.arg3 == 0 ? "открытый" :
	      MYCMD.arg3 == 1 ? "закрытый" : 
	      MYCMD.arg3 == 2 ? "запертый" : 
	      MYCMD.arg3 == 3 ? "скрытый"  : 
	      MYCMD.arg3 == 4 ? "явный"    : "неизвестно что"
	      );
      break;
    case 'T':
      sprintf(buf2, "%sпривязать тригер %s%s%s [%s%d%s] к %s",
        MYCMD.if_flag ? " тогда " : "",
        cyn, trig_index[real_trigger(MYCMD.arg2)]->proto->name, yel, 
        cyn, MYCMD.arg2, yel,
        ((MYCMD.arg1 == MOB_TRIGGER) ? "мобу" :
          ((MYCMD.arg1 == OBJ_TRIGGER) ? "предмету" :
            ((MYCMD.arg1 == WLD_TRIGGER)? "комнате" : "????"))));
      break;
    case 'V':
      sprintf(buf2, "%sприсвоить глобальную переменную %s:%d to %s = %s",
        MYCMD.if_flag ? " then " : "",
...
void zedit_disp_comtype(struct descriptor_data *d)
{
  ...
  sprintf(buf,
	...
	"[H[J"
#endif
	"%sM%s) Зачитать моба в комнату         %sO%s) Зачитать предмет в комнату\r\n"
	"%sE%s) Экипировать моба                %sG%s) Дать предмет мобуr\n"
...
void zedit_disp_arg1(struct descriptor_data *d)
{
  ...
  case 'M':
  case 'Q':
    send_to_char("Введите номер моба : ", d->character);
  ...
  case 'P':
  case 'G':
    send_to_char("Введите номер предмета : ", d->character);
    OLC_MODE(d) = ZEDIT_ARG1;
    break;
  case 'D':
  case 'R':
  ...
  case 'T':
  case 'V':
    send_to_char("Введите тип тригера (0:моб, 1:предмет, 2:комната) :", d->character);
    OLC_MODE(d) = ZEDIT_ARG1;
    break;
  default:
    /*
...
void zedit_disp_arg2(struct descriptor_data *d)
{
  ...
  {case 'M':
   ...
   case 'P':
   case 'G':
    send_to_char("Введите максимальное количество в мире : ", d->character);
    break;
   case 'D':
    while (*dirs[i] != '\n') 
    ...
    send_to_char("Введите направлене выхода : ", d->character);
    break;
  case 'R':
    send_to_char("Введите номер предмета : ", d->character);
    break;
  case 'F':
    send_to_char("Введите номер моба-лидера : ", d->character);
    break;
  case 'T':
    send_to_char("Введите номер тригера : ", d->character);
    break;
  case 'V':
    send_to_char("Глобальный контекст (0 - нет контекста) : ", d->character);
    break;
  case 'Q':
  default:
    /*
...
void zedit_disp_arg3(struct descriptor_data *d)
{
  ...
  {case 'E':
    ...
    send_to_char("Выберите позицию : ", d->character);
    break;
  case 'P':
    send_to_char("Vnum контейнера : ", d->character);
    break;
  case 'D':
    send_to_char("0)  Дверь открыта\r\n"
                 "1)  Дверь закрыта\r\n"
                 "2)  Дверь заперта\r\n"
                 "3)  Выход скрыт\r\n"
                 "4)  Выход явный\r\n"
                 "Введите состояние двери(выхода) : ", d->character);
    break;
  case 'V':
    send_to_char("Введите имя глобальной переменной: ", d->character);
    OLC_MODE(d) = ZEDIT_SARG1;
    return;
  case 'M':
    send_to_char("Введите максимальное количество в комнате : ", d->character);
    break;
  case 'F':
    send_to_char("Введите номер моба-последователя : ", d->character);
    break;
  case 'Q':
  case 'O':
  case 'R':
       ...
       {if (OLC_VAL(d)) 
           {/* If there was a previous command. */
            if (OLC_CMD(d).command == 'T' || OLC_CMD(d).command == 'V') 
               ...
               {send_to_char("Зависит ли эта команда от успешного выполнения предыдущей ? (y/n)\r\n", d->character);
...
/*-------------------------------------------------------------------*/
  case ZEDIT_ARG1:
     ...
     */
    if (!isdigit(*arg)) 
       {send_to_char("Должно быть числовое значение, повторите : ", d->character);
        return;
       }
    switch (OLC_CMD(d).command) 
    {case 'M':
     case 'Q':
       if ((pos = real_mobile(atoi(arg))) >= 0) 
	      ...
	      send_to_char("Нет такого моба, повторите : ", d->character);
       break;
     case 'O':
     case 'P':
     case 'E':
     case 'G':
       if ((pos = real_object(atoi(arg))) >= 0) 
	      ...
	      send_to_char("Нет такого предмета, повторите : ", d->character);
      break;
     case 'T':
     case 'V':
...
/*-------------------------------------------------------------------*/
  case ZEDIT_ARG2:
    ...
    case 'V':
      ...
      send_to_char("Введите глобальное имя : ", d->character);
      OLC_MODE(d) = ZEDIT_SARG1;
      break;
    ...
    case 'R':
      if ((pos = real_object(atoi(arg))) >= 0) 
	     ...
	     send_to_char("Нет такого предмета, повторите ввод : ", d->character);
      break;
    ...
    case 'T':
      if (real_trigger(atoi(arg)) >= 0) 
         ...
         send_to_char("Нет такого тригера, повторите ввод : ", d->character);
      break;
...
/*-------------------------------------------------------------------*/
  case ZEDIT_ARG3:
    ...
    if (!isdigit(*arg) && OLC_CMD(d).command != 'V') 
       {send_to_char("Должно быть числовое значение, повторите ввод : ", d->character);
        return;
       }
    switch (OLC_CMD(d).command) 
    {case 'M':
  ...
  case ZEDIT_SARG1:
    if (strlen(arg)) 
       {OLC_CMD(d).sarg1 = str_dup(arg);
        ...
        send_to_char("Введите значение глобальной переменной : ", d->character);
       } 
    else
       send_to_char("Вы должны указать имя переменной : ", d->character);
    break;
...
/*-------------------------------------------------------------------*/
  case ZEDIT_SARG2:
    if (strlen(arg)) 
       ...
       send_to_char("Вы должны указать значение переменной :", d->character);
    break;
...
/*-------------------------------------------------------------------*/
  case ZEDIT_ZONE_RESET:
    ...
    if (!isdigit(*arg) || (pos < 0) || (pos > 2))
       send_to_char("Повторите ввод (0-2) : ", d->character);
    else 
