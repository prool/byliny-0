...
{char names[MAX_SLOT][MAX_STRING_LENGTH];
 ...
 gcount = sprintf(buf2+gcount,"  %sВам доступна следующая магия :%s",
...
{ FILE *magic;
  ...
  while (get_line(magic,name))
        {if (!name[0] || name[0] == ';')
         ...
         if (!strn_cmp(line,"monoguild",strlen(line)) || !strn_cmp(line,"одноклассовая",strlen(line)))
            {type = 1;
             if (lines < 5)
          ...
          if (!strn_cmp(line,"polyguild",strlen(line)) || !strn_cmp(line,"многокласcовая",strlen(line)))
             {type       = 2;
              poly_guild = NULL;
              pgcount    = 0;
             }
          else
          if (!strn_cmp(line,"master",strlen(line)) || !strn_cmp(line,"учитель",strlen(line)))
...
SPECIAL(guild_mono)
{
  ...
  if (!IS_BITS(guild_mono_info[info_num].classes, GET_CLASS(ch)) ||
     ...
     {act("$N сказал$g : '$n, я не учу таких, как ты.'",
          FALSE, ch, 0, victim, TO_CHAR);
      return (1);
     }
...
{ case SCMD_LEARN:
  if (!*argument)
     {gcount += sprintf(buf,"Я могу научить тебя следующему:\r\n");
      ...
      if (!found)
         {act("$N сказал$G : 'Похоже, твои знания полнее моих.'",
              FALSE, ch, 0, victim, TO_CHAR);
          return (1);
         }
      else
...
{ struct char_data *victim = (struct char_data *) me, *horse = NULL;
  ...
  if (!strn_cmp(argument, "продать", strlen(argument)) ||
     ...
     {if (!has_horse(ch,TRUE))
         {act("$N засмеял$U : \"$n, ты не влезешь в мое стойло.\"",
              FALSE,ch,0,victim,TO_CHAR);
          return (TRUE);
         }
      if (on_horse(ch))
...
SPECIAL(bank)
{
  ...
  if (CMD_IS("deposit") || CMD_IS("вложить") || CMD_IS("вклад"))
  ...
  if (CMD_IS("withdraw") || CMD_IS("получить"))
     {if ((amount = atoi(argument)) <= 0)
