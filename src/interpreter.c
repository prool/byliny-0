...
"Вы создали новый персонаж.\r\n"
"\r\n"
/* "Помните, что теперь его имя останется с ним на все время игры,\r\n"
"посему, прочтите внимательно пожелания относительно выбранного Вами имени.\r\n"
"1) Вы - персонаж Киевской Руси, поэтому имена типа Гэндальф, Хоббит и т.п.\r\n"
"   могут быть относительно неуместны(но не запрещены)\r\n"
"2) Оставьте имена богов (Ярило, Перун и т.п.) для создателей - они могут быть\r\n"
"   их достойны :)\r\n"
"3) Имя Валенок - в принципе не возбраняется, но сами подумайте, каково Вам\r\n"
"   придется в компании Сапог, Самоваров и Топоров ?\r\n"
"4) Фамилии известных исторических личностей потребуют у Вас и соответствующих\r\n"
"   деяний, да и отношение к ним(личностям и деяниям) неоднозначно.\r\n"
"5) Если у персонажа с именем ХренВамВсем внезапно сила станет равна 3 или\r\n"
"   он не сможет ни по кому ни разу попасть - не удивляйтесь - Боги тоже имеют\r\n"
"   чувство юмора.\r\n"
"6) По техническим причинам имена из 2 слов и более не поддерживаются, для этих\r\n"
"   целей служит титул(который Вам еще предстоит заработать)\r\n"
"7) И последнее - для исключения разного рода недоразумений имя должно состоять\r\n"
"   не менее чем из 5 символов, которые не должны совпадать у разных игроков.\r\n"
"\r\n"
"   Итак, подойдите творчески к выбору имени, это только обогатит Ваши ощущения\r\n"
"от игры.\r\n"*/;
...
cpp_extern const struct command_info cmd_info[] = {
  ...
  { "автовыходы",	POS_DEAD,     do_gen_tog  , 0, SCMD_AUTOEXIT, 0 },
  ...
  { "возврат",		POS_DEAD    , do_return   , 0, 0, -1 },
  ...
  { "дата",		POS_DEAD    , do_date     , LVL_IMMORT, SCMD_DATE, 0 },
  ...
  { "заколоть",		POS_STANDING, do_backstab, 1, 0, 500 },
  ...
  { "замолчать",	POS_DEAD    , do_wizutil  , LVL_GOD, SCMD_SQUELCH, 0 },
  ...
  { "иммзаставка",	POS_DEAD    , do_gen_ps   , LVL_IMMORT, SCMD_IMOTD, 0 },
  ...
  { "краткий",		POS_DEAD    , do_gen_tog  , 0, SCMD_BRIEF, 0 },
  ...
  { "оглядеться",	POS_RESTING , do_sides    , 0, 0, 500 },
  ...
  { "осмотреть", 	POS_RESTING , do_examine  , 0, 0, 300 },
  ...
  // { "поздравить",	POS_RESTING , do_gen_comm , 0, SCMD_GRATZ, -1 },
  ...
  { "проснуться",	POS_SLEEPING, do_wake     , 0, 0, -1 },
  ...
  { "сжатый",			POS_DEAD    , do_gen_tog  , 0, SCMD_COMPACT, 0 },
  ...
  { "сохранить",		POS_SLEEPING, do_save     , LVL_IMPL, 0, 0 },
  ...
  { "усилить"       , POS_SLEEPING, do_force    , LVL_GRGOD, 0, 0 },
  ...
  { "экран"    , POS_DEAD    , do_toggle   , 0, 0, 0 },
...
{ int    load_room, load_result, neednews=0;
  ...
  if (lastnews && LAST_LOGON(d->character) < lastnews)
   ...
   //send_to_gods(buf);
   if (neednews)
      {sprintf(buf,"%sВас ожидают свежие новости (команда НОВОСТИ)%s\r\n",
                   CCWHT(d->character,C_NRM), CCNRM(d->character,C_NRM));
       send_to_char(buf,d->character);
      }
   look_at_room(d->character, 0);
   if (has_mail(GET_IDNUM(d->character)))
      send_to_char("Вас ожидает письмо.\r\n", d->character);
   if (load_result == 2)
      {/* rented items lost */
       send_to_char("\r\n\007Вы не смогли оплатить свой постой !\r\n"
                    "Все Ваши вещи переданы в помощь изнывающих от гнета игроков мобов !\r\n",
                    d->character);
      }
	...
	if (!Is_Valid_Dc(tmp_name))
	   {SEND_TO_Q("Игрок с подобным именем находится в игре.\r\n",d);
	    ...
	    if (cmp_ptable_by_name(tmp_name,MIN_NAME_LENGTH+1) >= 0)
	       {SEND_TO_Q("Первые  символов Вашего имени совпадают с уже существующим персонажем.\r\n"
...
printf("L2\n");
  ...
  case CON_GET_EMAIL:
    ...
    *(GET_EMAIL(d->character)+127) = '\0';
    ...
    sprintf(buf, "%s [%s] - Новый игрок - %s, %s.\r\n",
            GET_NAME(d->character), d->host,
  ...
  case CON_MENU:		/* get selection from main menu  */
    ...
    {
    case '0':
      SEND_TO_Q("До встречи на земле Киевской.\r\n", d);
      STATE(d) = CON_CLOSE;
      break;
  ...
  case CON_NAME6:
     ...
     if (!_parse_name(arg,tmp_name) &&
        ...
        {CREATE(GET_PAD(d->character,5), char, strlen(tmp_name)+1);
         ...
         sprintf(buf, "Введите пароль для %s : ", GET_PAD(d->character,1));
         SEND_TO_Q(buf, d);
         echo_off(d);
         STATE(d) = CON_NEWPASSWD;
        }
