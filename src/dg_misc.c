		 ...
		 int type, char *cmd)
{
  ...
  if (!caster) 
     {caster = read_mobile(DG_CASTER_PROXY, VIRTUAL);
      ...
      if (type==OBJ_TRIGGER)
         {sprintf(buf,"дух %s",((struct obj_data *)go)->PNames[1]);
          caster->player.short_descr = str_dup(buf);
          sprintf(buf,"дух %s",((struct obj_data *)go)->PNames[1]);
          GET_PAD(caster,0) = str_dup(buf);
          sprintf(buf,"духа %s",((struct obj_data *)go)->PNames[1]);
          GET_PAD(caster,1) = str_dup(buf);
          sprintf(buf,"духу %s",((struct obj_data *)go)->PNames[1]);
          GET_PAD(caster,2) = str_dup(buf);
          sprintf(buf,"духа %s",((struct obj_data *)go)->PNames[1]);
          GET_PAD(caster,3) = str_dup(buf);
          sprintf(buf,"духом %s",((struct obj_data *)go)->PNames[1]);
          GET_PAD(caster,4) = str_dup(buf);
          sprintf(buf,"духе %s",((struct obj_data *)go)->PNames[1]);
