...
{       "Христиане",      /* First, the alignment based ones */
        "Язычники",
        "Neutral",
		          ...
		          struct char_data * keeper, int shop_nr)
{
  ...
  if ((buy_price(obj, ch, shop_nr) > GET_GOLD(ch)) && !IS_GOD(ch))
     {sprintf(buf, shop_index[shop_nr].missing_cash2, GET_NAME(ch));
      ...
      {case 0:
            ...
            return;
       case 1:
           sprintf(buf,"отхлебнул$g немелкий глоток %s",
                   GET_SEX(keeper) == SEX_MALE ? "водки" : "медовухи");
  ...
  if (bought < buynum)
     {if (GET_GOLD(ch) < buy_price(obj, ch, shop_nr))
         sprintf(buf, "%s! Мошенни%s, ты можешь оплатить только %d.",
                 GET_NAME(ch), (GET_SEX(ch) == SEX_MALE) ? "к" : "ца", bought);
      else
      ...
      if (!obj || !same_obj(last_obj, obj))
         ...
         sprintf(buf, "%s! Я продам тебе только %d.", GET_NAME(ch),
                 bought);
      do_tell(keeper, buf, cmd_tell, 0);
     }
 ...
 /* sprintf(buf, shop_index[shop_nr].message_buy, GET_NAME(ch), goldamt); */
  ...
  sprintf(buf, "Теперь Вы стали %s %s.\r\n",
          GET_SEX(ch) == SEX_MALE ? "счастливым обладателем" :
                                    "счастливой обладательницей",
...
{ char buf[MAX_STRING_LENGTH];
  ...
  {
  case OBJECT_NOVAL:
    sprintf(buf, "%s! Не впаривай мне мозги, этот хлам можешь оставить себе !",
	        GET_NAME(ch));
    break;
  case OBJECT_NOTOK:
    if (mode == MODE_REPAIR)
 		                ...
 		                int shop_nr)
{
  ...
  if (GET_OBJ_MAX(obj) > GET_OBJ_CUR(obj) * 2)
     {sprintf(buf, "%s! Я не куплю %s в таком ужасном состоянии.",
  ...
  sprintf(buf, "$n продал$g %s.", tempstr);
  act(buf, FALSE, ch, obj, 0, TO_ROOM);
		        ...
		        struct char_data * keeper, int shop_nr)
{
  ...
  if ((sellnum = transaction_amt(arg)) < 0)
     {sprintf(buf, "%s! Этой вещи у тебя быть не может.",
  	          GET_NAME(ch));
      do_tell(keeper, buf, cmd_tell, 0);
      return;
     }
                     ...
                     int shop_nr, int operation)
{
 ...
 while (argument && *argument)
  ...
  if (!sellprice)
     {sprintf(buf, "%s! Ты не предлагаешь мне ничего, что мне требуется.",
              GET_NAME(ch));
      do_tell(keeper, buf, cmd_tell, 0);
      restore_objects(ch,source_list,srcs);
      return;
  ...
  if (bought < buynum)
     sprintf(buf,"%s У меня осталось всего %d %s.",GET_NAME(ch),
		         ...
		         struct char_data * keeper, int shop_nr)
{
  ...
  if (repair <= 0)
     {sprintf(buf,"%s! %s не нуждается в починке.",GET_NAME(ch),
              CAP(OBJN(obj,ch,0)));
      do_tell(keeper, buf, cmd_tell, 0);
      return;
     }
  ...
  if (price <= 0                    ||
      ...
      {sprintf(buf,"%s! Я не буду тратить свое драгоценное время на %s.",
               GET_NAME(ch), OBJN(obj,ch,3));
       do_tell(keeper, buf, cmd_tell, 0);
       return;
      }
		        ...
		        struct char_data * keeper, int shop_nr)
{
  ...
  one_argument(arg, name);
  strcpy(buf, " ##   Доступно    Предмет                                           Цена \r\n");
  strcat(buf, "-------------------------------------------------------------------------\r\n");
  if (keeper->carrying)
  ...
  if (!last_obj)
     {if (*name)
         strcpy(buf, "Ничего похожего для продажи нет.\r\n");
      else
         strcpy(buf, "Увы, товара пока нет.\r\n");
     }
  else
  if (!(*name) || isname(name, last_obj->name))
     strcat(buf, list_object(last_obj, cnt, index, shop_nr));
...
SPECIAL(shop_keeper)
{
  ...
  if (CMD_IS("list") || CMD_IS("список"))
     {shopping_list(argument, ch, keeper, shop_nr);
      return (TRUE);
     }
  else
  if (CMD_IS("repair") || CMD_IS("чинить"))
...
void list_detailed_shop(struct char_data * ch, int shop_nr)
{
  ...
  sprintf(buf, "К.покупки:[%4.2f], К.обмена:[%4.2f], К.продажи:[%4.2f], открыт: [%d-%d, %d-%d]%s",
