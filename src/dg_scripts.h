...
#define MAX_SCRIPT_DEPTH        128         /* maximum depth triggers can
...
void income_mtrigger(struct char_data *actor, int dir);
...
int  damage_mtrigger(struct char_data *damager, struct char_data *victim);
...
int open_wtrigger(struct room_data *room, char_data *actor, int dir, int unlock);
int close_wtrigger(struct room_data *room, char_data *actor, int dir, int lock);
