...
char *read_delete(long recipient)
{
  ...
  *(tmstr + strlen(tmstr) - 1) = '\0';
  ...
  sprintf(buf, " * * * * Почтовая служба князя * * * *\r\n"
	      "Date: %s\r\n"
	      "  To: %s\r\n"
	      "From: %s\r\n\r\n", 
	      tmstr, to ? to : "Получатель",  from ? from : "Аноним");
...
SPECIAL(postmaster)
{
  ...
  if (CMD_IS("check") || CMD_IS("почта")) 
     {postmaster_check_mail(ch, (struct char_data *)me, cmd, argument);
      return (1);
     } 
  else 
  if (CMD_IS("receive") || CMD_IS("получить")) 
			  ...
			  int cmd, char *arg)
{
  ...
  if (GET_LEVEL(ch) < MIN_MAIL_LEVEL) 
     {sprintf(buf, "$n сказал$g Вам, 'Извините, Вы должны достигнуть %d уровня, чтобы отправить письмо!'",
	    MIN_MAIL_LEVEL);
  ...
  if (!*buf) 
     {/* you'll get no argument from me! */
      act("$n сказал$g Вам, 'Вы не указали адресата!'",
  ...
  if ((recipient = get_id_by_name(buf)) < 0) 
     {act("$n сказал$g Вам, 'Извините, но такого игрока нет в игре!'",
	      FALSE, mailman, 0, ch, TO_VICT);
      return;
     }
  act("$n начал$g писать письмо.", TRUE, ch, 0, 0, TO_ROOM);
  sprintf(buf, "$n сказал$g Вам, 'Отлично, с Вас %d %s почтового сбора.'\r\n"
