  ...
  if (IS_NPC(ch))
     ...
     {if (real_sector(ch->in_room)            == SECT_WATER_NOSWIM ||
      ...
      if (on_horse(ch) && !legal_dir(get_horse(ch), dir, need_specials_check, FALSE))
         { if (show_msg)
              act("Ваш$G $N отказывается туда идти.\r\n",FALSE,ch,0,get_horse(ch),TO_CHAR);
           return (FALSE);
         }

      if (on_horse(ch) && GET_HORSESTATE(get_horse(ch)) <= 0)
         {if (show_msg)
             act("Ваш$G $N загнан$G настолько, что не может нести Вас на себе.",FALSE,ch,0,get_horse(ch),TO_CHAR);
          return (FALSE);
         }
          ...
          for (k = ch->followers; k && k->follower->master; k = next)
		    ...
		    (!PLR_FLAGGED(k->follower,PLR_MAILING) &&
		     !PLR_FLAGGED(k->follower,PLR_WRITING)
	           ...
	           (!IS_HORSE(k->follower) ||
                    !AFF_FLAGGED(k->follower, AFF_TETHERED)
                   )
                  )
...
void do_doorcmd(struct char_data *ch, struct obj_data *obj, int door, int scmd)
{
  ...
  sprintf(buf + strlen(buf), "%s.",
          (obj) ? "$p" : (EXIT(ch, door)->keyword ? "$F" : "дверь"));
  ...
  if ((scmd == SCMD_OPEN || scmd == SCMD_CLOSE) && back)
     {sprintf(buf, "$N %s %s.",
	           cmd_door[scmd],
	           (back->keyword ? fname(back->keyword) : "дверь"));
  ...
  if (scmd == SCMD_PICK)
     {
      ...
      if (percent > train_skill(ch, SKILL_PICK_LOCK,skill_info[SKILL_PICK_LOCK].max_percent,NULL))
...
ACMD(do_gen_door)
{
  ...
  if ((obj) || (door >= 0))
     { keynum = DOOR_KEY(ch, obj, door);
       ...
       if (!has_key(ch, keynum) && !IS_GOD(ch) &&
...
ACMD(do_sleep)
{
  ...
  default:
    ...
    act("$n прекратил$g летать и нагло заснул$g.",
	TRUE, ch, 0, 0, TO_ROOM);
    GET_POS(ch) = POS_SLEEPING;
    break;
      }
...
{ struct char_data *horse;
  ...
  if (AFF_FLAGGED(horse, AFF_TETHERED))
     ...
     {if (affected_by_spell(ch, AFF_SNEAK))
         affect_from_char(ch, SPELL_SNEAK);
      if (affected_by_spell(ch,AFF_CAMOUFLAGE))
         affect_from_char(ch, SPELL_CAMOUFLAGE);
     ...
     {if (!IS_IMMORTAL(ch) && !(GET_CLASS(ch) == CLASS_THIEF))
      ...
      if (!IS_IMMORTAL(ch))
         {if (!GET_SKILL(ch,SKILL_STEAL))
          ...
          if (percent > prob)
             {act("Вы неудачно попытались украсть скакуна у $N1.",FALSE,ch,0,horse->master,TO_CHAR);
              act("$n неудачно попытал$u украсть скакуна у $N1.",TRUE,ch,0,horse->master,TO_NOTVICT);
